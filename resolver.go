...
func dryAPICall(query string, clientAddress string, qname string) {
	...
	if ((currentTime - lastStamp)*1000 > settings.Backend.SleepWhenDisabled) {
		...
		if (err != nil) {
			Info("Core remains DISABLED. Gonna wait. Error: %s", err)
			atomic.StoreInt64(&disabledSecondsTimestamp, int64(time.Now().Unix()))
			return
		}
		if (elapsed > time.Duration(settings.Backend.FitResponseTime)*time.Millisecond) {
			Info("Core remains DISABLED. Gonna wait. Elapsed time: %s, FitResponseTime: %s", elapsed, time.Duration(settings.Backend.FitResponseTime)*time.Millisecond)
			atomic.StoreInt64(&disabledSecondsTimestamp, int64(time.Now().Unix()))
			return
		}
		...
		Debug("Not enough time passed, waiting for another call. Elapsed: %s ms, Limit: %s ms", (currentTime - lastStamp)*1000, settings.Backend.SleepWhenDisabled)
	}
	return
}
...
func sinkitBackendCall(query string, clientAddress string, trimmedQname string) (bool) {
	...
	if (err != nil) {
		...
		atomic.StoreInt64(&disabledSecondsTimestamp, int64(time.Now().Unix()))
		Info("Core was DISABLED. Error: %s", err)
		return false
	}
	if (elapsed > time.Duration(settings.Backend.FitResponseTime)*time.Millisecond) {
		atomic.StoreUint32(&coreDisabled, 1)
		atomic.StoreInt64(&disabledSecondsTimestamp, int64(time.Now().Unix()))
		Info("Core was DISABLED. Elapsed time: %s, FitResponseTime: %s", elapsed, time.Duration(settings.Backend.FitResponseTime)*time.Millisecond)
		return false
	}
		...
		for i := range vals {
			...
			if (strings.EqualFold(vals[i], "A") || strings.EqualFold(vals[i], "CNAME") || strings.EqualFold(vals[i], "AAAA")) {
				...
				if (len(vals) > i+1 && len(vals[i+1]) > 3 && !strings.HasSuffix(vals[i+1], ".root-servers.net.")) {
					Debug("KARMTAG: to send to Core: %s\n", vals[i+1])
